...
class RegisterScreen {
	...
	constructor() {
		...
		this.$link.innerText = 'I have an account!';
		this.$link.classList.add('d-block', 'link');
		this.$link.addEventListener('click', this.handleChangeScreen);

		this.$userName = new InputComponent('Username', 'user', 'userName', 'text');
	...
	handleSubmit = async (e) => {
		e.preventDefault();
		const { email, password, rePassword, username } = e.target;
		let isError = false;
		...
		if (checkUserName(username.value) !== null) {
			// loi
			this.$email.setError(checkUserName(username.value));
			isError = true;
		} else {
			this.$email.setError('');
		}
